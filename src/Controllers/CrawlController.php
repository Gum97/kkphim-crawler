...
class CrawlController extends CrudController
{
        ...
        try {
            ...
            foreach ($request['link'] as $link) {
                if (preg_match('/(.*?)(\/phim\/)(.*?)/', $link)) {
                    $link = sprintf('%s/phim/%s', config('kkphim.domain', 'https://apii.online/kkphim'), explode('phim/', $link)[1]);
    ...
    {
        $pattern = sprintf('%s/phim/{slug}', config('kkphim_crawler.domain', 'https://apii.online/kkphim'));
        try {
